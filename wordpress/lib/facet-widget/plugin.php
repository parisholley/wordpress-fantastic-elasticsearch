...
class Taxonomies_Filter_Widget extends \WP_Widget {
	...
	public function register_admin_scripts_styles($hook) {
		...
		if( 'widgets.php' != $hook )
        	return;
		wp_enqueue_script( 'taxonomies-filter-widget-admin-script', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/js/admin.js' ), array('jquery'), false, true );
		wp_enqueue_style( 'taxonomies-filter-widget-admin-styles', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/css/admin.css' ) );
	...
	public function register_widget_scripts() {
		...
		wp_enqueue_script( 'taxonomies-filter-widget-slider-script', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/js/jquery.nouislider.min.js' ), array('jquery'), false, true );
		wp_enqueue_script( 'taxonomies-filter-widget-script', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/js/widget.js' ), array('jquery'), false, true );
	...
	public function register_widget_styles() {
		wp_register_style( 'taxonomies-filter-widget-styles', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/css/widget.css' ) );
		wp_register_style( 'taxonomies-filter-widget-slider-styles', plugins_url( ES_PLUGIN_DIR.'/wordpress/lib/facet-widget/css/nouislider.fox.css' ) );
	...
	public function print_dropdown_taxonomy($taxonomy, $parent = 0, $selected = '', $echo = 1, $name){      
		...
		if ( taxonomy_exists($taxonomy) ) { 
			...
			$args = array(
				...
				'tab_index'          => 0,
				'taxonomy'           => $taxonomy,
				'hide_if_empty'      => Api::option('hide_empty'),
