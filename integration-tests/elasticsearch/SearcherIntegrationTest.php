...
class SearcherIntegrationTest extends BaseIntegrationTestCase
{
	...
	public function testSearchNotScored()
	{
		...
		$results = Searcher::search('value1');
	...
	public function testSearchFields()
	{
		...
		$results = Searcher::search('value2');

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(1, 2), $results['ids']);

		$results = Searcher::search('value1 value3');

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(1, 2), $results['ids']);

		$results = Searcher::search('value3');

		$this->assertEquals(1, $results['total']);
		$this->assertEquals(array(2), $results['ids']);

		$results = Searcher::search('value4');
	...
	public function testSearchPaging()
	{
		...
		$results = Searcher::search('value1', 0, 1);

		$this->assertEquals(3, $results['total']);
		$this->assertEquals(array(1), $results['ids']);

		$results = Searcher::search('value1', 1, 1);

		$this->assertEquals(3, $results['total']);
		$this->assertEquals(array(2), $results['ids']);

		$results = Searcher::search('value1', 2, 1);
	...
	public function testSearchTaxonomies()
	{
		...
		$results = Searcher::search(null, 0, 10, array('tag' => 'tag1'));

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(1, 3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 1, 'tag1' => 2)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array('tag1')));

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(1, 3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 1, 'tag1' => 2)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => 'tag2'));

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(2, 3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 2, 'tag1' => 1)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array('tag2')));

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(2, 3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 2, 'tag1' => 1)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array('tag1', 'tag2')));

		$this->assertEquals(1, $results['total']);
		$this->assertEquals(array(3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 1, 'tag1' => 1)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array( 'and' => array('tag1', 'tag2'))));

		$this->assertEquals(1, $results['total']);
		$this->assertEquals(array(3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 1, 'tag1' => 1)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array('tag1', 'tag3')));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array(), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array( 'and' => array('tag1', 'tag3'))));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array(), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array( 'or' => array('tag1', 'tag2'))));

		$this->assertEquals(3, $results['total']);
		$this->assertEquals(array(1, 2, 3), $results['ids']);
		$this->assertEquals(array('tag' => array('tag2' => 2, 'tag1' => 2)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('tag' => array( 'or' => array('tag1', 'tag3'))));
	...
	public function testSearchRangeSegments()
	{
		...
		$results = Searcher::search(null, 0, 10, array('field1' => '-10'));

		$this->assertEquals(1, $results['total']);
		$this->assertEquals(array(1), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 1, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => '10-20'));

		$this->assertEquals(2, $results['total']);
		$this->assertEquals(array(2, 3), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 2, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array('-10', '10-20')));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'and' => array('-10', '10-20'))));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'or' => array('-10', '10-20'))));

		$this->assertEquals(3, $results['total']);
		$this->assertEquals(array(1, 2, 3), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 1, '10-20' => 2, '20-' => 3)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => '20-'));

		$this->assertEquals(3, $results['total']);
		$this->assertEquals(array(4, 5, 6), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 3)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array('-10', '20-')));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'and' => array('-10', '20-'))));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array('10-20', '20-')));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'and' => array('10-20', '20-'))));

		$this->assertEquals(0, $results['total']);
		$this->assertEquals(array(), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 0, '10-20' => 0, '20-' => 0)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'or' => array('10-20', '20-'))));

		$this->assertEquals(5, $results['total']);
		$this->assertEquals(array(4, 5, 6, 2, 3), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 1, '10-20' => 2, '20-' => 3)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'or' => array('-10', '20-'))));

		$this->assertEquals(4, $results['total']);
		$this->assertEquals(array(4, 5, 1, 6), $results['ids']);
		$this->assertEquals(array('field1' => array('-10' => 1, '10-20' => 2, '20-' => 3)), $results['facets']);

		$results = Searcher::search(null, 0, 10, array('field1' => array( 'or' => array('-10', '10-20', '20-'))));
	...
	public function testSearchRangeLadderUp()
	{
		...
		$results = Searcher::search(null, 0, 10, array('field1' => '0-'));
	...
	public function testSearchRangeLadderDown()
	{
		...
		$results = Searcher::search(null, 0, 10, array('field1' => '-30'));
