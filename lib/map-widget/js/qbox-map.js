(function(){
  ...
  var server_url = 'http://184.72.234.55:9200/';
  var ELASTICSEARCH_INDEX_ENDPOINT = server_url;
  ...
  var resultTemplate = Handlebars.compile('\
    <tr class="result-row" id={{id}}>\
      <td>{{insurance_plan_category}}</td>\
      <td>{{aca_plan_level}}</td>\
      <td>{{insurance_plan_feature}}</td>\
      <td>{{formatted_lat}}</td>\
      <td>{{formatted_lon}}</td>\
    </tr>\
  ');
